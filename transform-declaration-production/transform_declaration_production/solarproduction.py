...
class SolarProduction(beam_core.DoFn, ABC):
    ...
    def __transform_content(content):
        ...
        df.rename(columns={'Qnt': 'Solar', 'TimestampUTC': 'HourUTC'}, inplace=True)

        df = df[["HourUTC", "PriceArea", "Solar"]]
        df['HourUTC'] = pd.to_datetime(df['HourUTC'])

        return df.groupby(["HourUTC", "PriceArea"]).sum()
    ...
    def process(self, element, *args, **kwargs):
        ...
        try:
            ...
            if content is not None:
                element['solar'] = self.__transform_content(content)
                element['solar'].to_csv("c:\\temp\\solar.csv", sep=";", decimal=",")
                return [element]
