...
class ForeignExchangeAdapter(IngressAdapter):
    ...
    def retrieve_data(self) -> Optional[bytes]:
        ...
        """
        logger.debug('Running the Regulated Consumption Ingress Adapter')

        select = "UTC_HOUR_TEXT_FULL,PRICE_AREA_CODE,READING_VALUE"

        dataframe = self.service.get_data_as_dataframe(self.start_date, self.end_date, 'RFB', 'QUARTER', select)
...
def ingest_regulated_consumption_data(start_date: datetime):
    """
...
if __name__ == "__main__":
    ...
    while start_date < end_date:
        ...
        ingest_regulated_consumption_data(start_date)
