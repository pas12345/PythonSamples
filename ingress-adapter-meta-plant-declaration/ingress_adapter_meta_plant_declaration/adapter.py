"""
Meta-Units Adapter for Ingress
"""
...
class MetaunitsAdapter(IngressAdapter):
    """
    The Meta-Units Adapter.
    ...
    def retrieve_data(self) -> Optional[bytes]:
        """
        Retrieves the data from Meta-Units.
        """
        logger.debug('Running the Meta-Units Ingress Adapter')

        # TODO: Implement code to retrieve data and return it as a bytes string.
        filename = config['Source files']['plant_declaration']
...
def ingest_meta_units_data():
    """
    Setups the adapter and runs it.
    """
    adapter = MetaunitsAdapter(config['Azure Storage']['ingress_url'],
